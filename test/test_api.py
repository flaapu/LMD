...
def test_contract_type_invalido_retorna_422(client):
    cliente = CLIENTE_VALIDO.copy()
    cliente["contract_type"] = "trimestral"  # no es una categoría válida
    response = client.post("/predict", json=cliente)
    assert response.status_code == 422


def test_payment_method_invalido_retorna_422(client):
    cliente = CLIENTE_VALIDO.copy()
    cliente["payment_method"] = "criptomoneda"  # no es una categoría válida
    response = client.post("/predict", json=cliente)
    assert response.status_code == 422


def test_region_invalida_retorna_422(client):
    cliente = CLIENTE_VALIDO.copy()
    cliente["region"] = "patagonia"  # no es una categoría válida
    response = client.post("/predict", json=cliente)
    assert response.status_code == 422


# ── Monitoreo técnico (Examen final) ──────────────────────────

def test_metrics_endpoint_expone_metricas_prometheus(client):
